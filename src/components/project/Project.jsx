...
import {useAddToWishlistMutation} from "../../store/projectsApi";
import location from "./../../images/services/marker-location.svg";

const Project = ({title, img, year, description, id}) => {
    const [addToWishlist] = useAddToWishlistMutation();
    
    const addWishlist = async(body) => {
        await addToWishlist({id: id, title: title, img: img, year: year, description: description}).unwrap();
    }
        ...
        <>
            ...
            <li className="project">
                <NavLink to={`/project/${id}`}>
                    <img src={process.env.PUBLIC_URL + img} alt={title} className="project-image" />
                </NavLink>
